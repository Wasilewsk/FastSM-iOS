...
internal class MastodonStreaming(
    ...
    fun events(): Flow<TimelineEvent> = channelFlow {
        ...
        while (isActive) {
            ...
            if (!isActive) break
            if (result.isSuccess) backoffMs = 1_000L
            delay(backoffMs)
            backoffMs = (backoffMs * 2).coerceAtMost(30_000L)
        }
    }

    private suspend fun runSession(scope: ProducerScope<TimelineEvent>, token: String) {
        val wsUrl = buildString {
            append(instanceBase.replaceFirst("http://", "ws://").replaceFirst("https://", "wss://"))
            append("/api/v1/streaming?stream=user&access_token=").append(token)
        }
    ...
    private fun parse(raw: String): TimelineEvent? {
        val env = runCatching { json.decodeFromString(MastodonStreamEnvelope.serializer(), raw) }.getOrNull() ?: return null
