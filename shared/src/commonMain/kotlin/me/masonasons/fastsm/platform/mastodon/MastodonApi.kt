...
class MastodonApi(
    ...
    suspend fun exchangeToken(
        clientId: String, clientSecret: String, code: String, redirectUri: String, scopes: String,
    ): MastodonTokenDto =
        httpClient.submitForm(
    ...
    suspend fun getAccountStatuses(userId: String, limit: Int, maxId: String?, excludeReplies: Boolean): List<MastodonStatusDto> =
    ...
    private fun nextMaxIdFromLink(link: String?): String? {
        ...
        val queryPart = urlPart.substringAfter('?', "")
    ...
    suspend fun postStatus(
        status: String, inReplyToId: String?, visibility: String?, spoilerText: String?,
        mediaIds: List<String>, pollOptions: List<String> = emptyList(), pollExpiresInSec: Int? = null,
        pollMultiple: Boolean = false, pollHideTotals: Boolean = false, quotedStatusId: String? = null,
    ): MastodonStatusDto =
        httpClient.submitForm(
    ...
    suspend fun scheduleStatus(
        status: String, scheduledAtIso: String, inReplyToId: String?, visibility: String?,
        spoilerText: String?, mediaIds: List<String>, pollOptions: List<String> = emptyList(),
        pollExpiresInSec: Int? = null, pollMultiple: Boolean = false, pollHideTotals: Boolean = false,
    ...
    suspend fun uploadMedia(bytes: ByteArray, filename: String, mime: String, description: String?): MastodonMediaDto =
    ...
    suspend fun updateMediaDescription(mediaId: String, description: String): MastodonMediaDto =
        httpClient.put("$instanceBase/api/v1/media/$mediaId") {
            bearer()
            setBody(FormDataContent(Parameters.build { append("description", description) }))
        }.body()

    suspend fun editStatus(statusId: String, status: String, spoilerText: String?): MastodonStatusDto =
